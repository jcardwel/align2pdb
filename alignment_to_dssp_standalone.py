...
## collect args
argString = "%s -d fasta_input_directory -c chain_file"%sys.argv[0]
try:
    optlist, args = getopt.getopt(sys.argv[1:],'hd:c:')
except getopt.GetoptError:
    print('alignment2dssp -d <fasta_input_directory> -c <chain_file>')
...
def check_args():
    ...
    if path == '':
        print("="*60)
        print('usage: alignment2dssp -d <fasta_input_directory> -c <chain_file>')
        print("="*60)
        sys.exit()
    if chain_file == '':
        print("="*60)
...
def create_directories():
    ...
    if os.path.isdir("xml_files/") == False:
        print("creating SIFTS XML file directory")
        subprocess.call(["mkdir", "xml_files"])
...
def sifts_parse(record):
    ...
    if os.path.isfile("xml_files/"+xml_file) == False:
        try:
            ...
            subprocess.call(["mv", xml_file,"xml_files/"])
        except:
            pass
    else:
...
if __name__ == "__main__": 
    ...
    for folder in folders:
        ...
        for fasta in fastas:
            ...
            filename = fasta
            input_for_aligner = fasta_parser(path+"/"+folder+"/"+filename)
            ...
            results.to_csv("output/"+filename.split(".fas")[0]+"_dssp.txt",sep="\t",index=False)
            summarized_results=get_best_ss(results)
            summarized_results.to_csv("output/"+filename.split(".fas")[0]+"_summarized.txt",sep="\t",index=False)
